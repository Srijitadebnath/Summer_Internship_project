...
export const PipelineCatalogueView: React.FC<PipelineCatalogueViewProps> = ({ onBack }) => {
  const [selectedCategory, setSelectedCategory] = useState<string>('All');
  const [selectedBrand, setSelectedBrand] = useState<string>('All');
  const [searchQuery, setSearchQuery] = useState<string>('');

  const categoriesList = ['All', 'CPVC Pipes', 'UPVC Fittings', 'Valves & Controls', 'SWR Drainage'];
  const brandsList = ['All', 'SUPREME', 'FINOLEX'];
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      if (!product.id.startsWith('p')) return false;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-neutral-cool">
          ...
          <h3 className="font-display font-extrabold text-lg text-primary uppercase tracking-wide">
            Pipeline & Fittings Showcase
          </h3>
        </div>

        {/* 2. Video Banner */}
        <CatalogueVideoBanner
          title="Pipeline & Fittings Catalogue"
          subtitle="Explore our industry-grade plumbing solutions. We are authorized distributors of Supreme and Finolex, delivering durable CPVC/UPVC networks for hot, cold, and drainage systems."
          videoSrc="/videos/PipelineandFittings.mp4"
          category="Live Technical Showcase"
        />
        ...
        <div className="bg-neutral-cool/40 border border-neutral-cool/80 rounded-3xl p-4 sm:p-5 mb-6 flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
          ...
          <div className="relative flex-grow max-w-md">
            <input
              type="text"
              placeholder="Search catalog (e.g. CPVC, Supreme, Elbow)..."
              value={searchQuery}
